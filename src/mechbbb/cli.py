    ...
    if args.smiles:
        ...
        for r in results:
            rows.append({
                ...
                "prob_std_error": f"{r.prob_std_error:.6f}" if (r.is_valid and r.prob_std_error is not None) else "",
                "prob_std_error_pct": f"{r.prob_std_error * 100:.2f}%" if (r.is_valid and r.prob_std_error is not None) else "",
                "prob_CI_lower": f"{max(0.0, r.prob - 2 * r.prob_std_error):.6f}" if (r.is_valid and r.prob_std_error is not None) else "",
                "prob_CI_upper": f"{min(1.0, r.prob + 2 * r.prob_std_error):.6f}" if (r.is_valid and r.prob_std_error is not None) else "",
